...
const productLinks = ["Industrial Hoses and Pipes", "Industrial Gaskets", "Rubber Products"];

const ContactInfo = ({ emailSize = "text-xs" }: { emailSize?: string }) => (
  <ul className="space-y-2 text-xs text-primary-foreground/70">
    ...
    <li className="flex items-start gap-1.5 min-w-0">
      ...
      <span className={`break-all min-w-0 ${emailSize}`}>info@duponindustrial.com</span>
    </li>
    <li className="flex items-start gap-1.5 min-w-0">
      <Mail size={12} className="mt-0.5 shrink-0" />
      <span className={`break-all min-w-0 ${emailSize}`}>sales@duponindustrial.com</span>
    </li>
  </ul>
);
  ...
  <footer className="gradient-navy text-primary-foreground overflow-hidden">
    <div className="container-wide mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {/* Col 1: Logo + Company Profile (mobile left, desktop col 1) */}
        <div className="col-span-1">
          ...
          {/* Quick Links shown below logo on mobile only */}
        ...
        {/* Col 2 (mobile right): Products + Contact stacked */}
        <div className="col-span-1 lg:hidden min-w-0">
          <h4 className="font-heading font-semibold text-sm mb-3">Products</h4>
          <ul className="space-y-1.5 text-xs text-primary-foreground/70 mb-4">
            {productLinks.map((p) => (
          ...
          <ContactInfo emailSize="text-[10px] sm:text-xs" />
        </div>

        {/* Desktop col 2: Quick Links */}
